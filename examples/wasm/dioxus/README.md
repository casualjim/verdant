# `verdant` Dioxus Example

This folder contains an example project showcasing use of `verdant` with
[`Dioxus`](https://dioxuslabs.com/).

The demo can be found online
[here](https://rubixdev.github.io/verdant/demos/dioxus/).

When targeting `wasm32-unknown-unknown` as required by wasm-pack/wasm-bindgen
(which is used by dioxus), the `verdant-parsers-git` collection must be used.
...
`verdant` dependencies. It is also recommended to only enable the parsers
