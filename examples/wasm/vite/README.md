# `verdant` Vite + Svelte + TypeScript Example
...
[here](https://rubixdev.github.io/verdant/demos/vite/).
...
By default, `verdant` fails to load its WebAssembly file correctly when
running Vite's development server. To work around that, we can manually specify
the URL to the Wasm file when initializing `verdant` and make use of Vite's
